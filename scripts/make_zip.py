...
def  main():
  ...
  installer = "\installer\Transmitter Installer.exe"
   
  if demo:
    installer = "\installer\Transmitter Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Transmitter manual.pdf" 
  ]
  ...
  ZIPNAME = "Transmitter-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Transmitter-v" + FULLVERSIONSTR + "-win-demo.zip"
